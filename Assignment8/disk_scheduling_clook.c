...
void serve_requests_clook(struct Request request_array[], int number_of_requests, int head)
{
    /*
     *  Input   :   Request array, number of requests, Initial head position.
     *  Utility :   Generate Sequence for serving requests using C-LOOK algorithm.
    ...
    // Display succeeding tracks in descending order
    for(int i = succeding_count - 1; i >= 0; i--)
    {
...
int main()
{
    ...
    serve_requests_clook(request_array, number_of_requests, head);
...
varadmash@varadmash-G3-3590:~/OS_Lab/Assignment8$ gcc disk_scheduling_clook.c 
...
Serving Sequence : 50 -> 34 -> 11 -> 180 -> 123 -> 119 -> 95 -> 64 -> 62
Total Head Movements : 157s
